...
def train_epoch(model, opt, loss_fn, data_tr, patch_v, device=DEVICE):
...
def eval_epoch(model, loss_fn, data_val, patch_v, DEVICE=DEVICE):
...
def train(model, opt, loss_fn, epochs, data_tr, data_val, batch, path_to_save, patience=10, patch_v=False,
    ...
    with tqdm(desc="epoch", total=epochs) as pbar_outer:
        for epoch in range(epochs):
            train_acc, train_loss = train_epoch(model, opt, loss_fn, data_tr, patch_v,
                                                DEVICE)
            print('* Epoch %d/%d' % (epoch + 1, epochs))
            pbar_outer.update(1)
            test_acc, val_loss = eval_epoch(model, loss_fn, data_val, patch_v,
                                            DEVICE)
